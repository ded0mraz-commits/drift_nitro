				...
				if GetPedInVehicleSeat(GetCar(), -1) == GetPed() then
					...
					if CarSpeed <= drift_speed_limit then  
						...
						if IsControlPressed(1, 21) then
							...
							SetVehicleEngineTorqueMultiplier(GetCar(), 4.5)
							SetVehicleEnginePowerMultiplier(GetCar(), 2.4)
						else
							SetVehicleEngineTorqueMultiplier(GetCar(), 1.0)
							SetVehicleEnginePowerMultiplier(GetCar(), 1.0)
							StopScreenEffect('ChopVision', 0, false)
						end
					end
				end
			end
